...
int main()
{
    ...
    printf("\nThe full path is: %s\n", full_path); //Print path information
    ...
    while(temp)                            //parsing path into 2_d array
	{
		...
		printf("\n The path being added to the 2d array is %s\n", temp);
	...
	for(;;)                         //service loop until the user quits
	{
		...
		if(fork() == 0)
		{
			char *Local_Command; 					            //Make a cstring to hold local command
 			...
 			printf("The Local Command is: %s\n",Local_Command); //Print the local command
			...
			while(argumentcounter < 3 )               //parsing up to 3 args into 2D array
			{
                ...
                printf("Adding %s to the 2d array of args",Local_Argument[argumentcounter]);
            ...
            for(k = 0; k < pathcounter; k++)
            {
                ...
                printf("The path about to be checked is %s\n",path);
